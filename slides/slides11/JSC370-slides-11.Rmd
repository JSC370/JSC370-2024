...
subtitle: "JSC 370: Data Science II"
...
```{r echo=FALSE, out.width="65%", fig.align="center", fig.cap="Where interactive visualization fits in to the data science workflow"}
knitr::include_graphics("img/wrangle.png")
```
...
<div style="text-align: center;">
  <figure>
    <img style="width: 350px;vertical-align: middle;"  hspace="20px" src="https://i0.wp.com/www.worklearning.com/wp-content/uploads/2015/01/Cone_of_learning_export_from_Wikipedia_11-11-2007.jpg?resize=705%2C518&amp;ssl=1" alt="Edgar Dale Cone of Experience"
  </figure>
...
<div style="text-align: center;">
  ...
  <figure>
    <img style="width: 500px;vertical-align: middle;"  hspace="20px" src="img/COVID-uni-testing.png" alt="EpiModel University Testing"
  </figure>
...
<div style="text-align: center;">
  ...
  <figure>
    <img style="width: 480px;vertical-align: middle;"  hspace="20px" src="img/atlas-of-inequality.png" alt="MIT-inequality"
  </figure>
...
<div style="text-align: center;">
  <figure>
    <img style="width: 100px;vertical-align: middle;"  hspace="5px" src="img/plotly.png" alt="Plotly"
  </figure>
</div>
...
### Load data for examples 
We have preprocessed the data for this presentation and added variables to `"input_data/coronavirus_states.csv"`
  ...
  - `naive_CFR`: naive Case Fatality Rate (CFR) = deaths / cases, for each state on each date
  
"naive" calculation of CFR requires more sophisticated modeling to account for time delays between cases and deaths
...
p <- ggplot(cv_states_today, aes(x=deathsper100k, y=per100k, size=naive_CFR)) + geom_point() + geom_smooth()
p1<-ggplotly(p)
widgetframe::frameWidget(p1, width = '70%')
```

<br>
<br>

`geom_smooth()` helps us to see there is not a clear correlation between cases and deaths relative to population across the states. Note that the `geom_smooth()` line also appears upon hover.
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
p <- ggplot(cv_states_today, aes(x=deathsper100k, y=per100k)) + geom_point() + geom_smooth()
fig <- p %>%
  ggplotly(layerData = 2, originalData = F) %>%
  add_fun(function(fig) {
    fig %>% slice(which.max(se)) %>%
